...
These screenshots are generated by TuiSpec via
[`scripts/generate-snapshots.sh`](scripts/generate-snapshots.sh). The app
itself uses terminal-default colors; these screenshots are rendered with
TuiSpec's dark and light terminal palettes. The dark shot uses a wider viewport
to show the grid layout, while the light shot keeps the narrow one-column
layout.
